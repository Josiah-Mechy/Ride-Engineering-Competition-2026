# Ride Engineering Competition 2026 – **Joop Adventures: Jungle Escape**

![Ride Model](https://raw.githubusercontent.com/Josiah-Mechy/Ride-Engineering-Competition-2026/main/Ride%20Engineering%20Competition%20Day%20Picture.jpg)

*A fully engineered tabletop roller coaster designed for the 2026 Ride Engineering Competition (REC) by the Liberty University Theme Park Engineering Club.*

---

# Overview

**Joop Adventures: Jungle Escape** is a competition-scale roller coaster that was designed, analyzed, manufactured, and validated for the 2026 Ride Engineering Competition. The project required developing a portable amusement attraction while meeting strict engineering constraints for rider safety, structural integrity, manufacturability, reliability, and operational performance.

As the **Design Team Lead** and **Project Manager**, I directed the mechanical design process, coordinated project milestones, and led the engineering validation of the ride. My responsibilities included the majority of the CAD development, engineering calculations, finite element analysis, and technical documentation required to bring the project from concept to competition.

---

### 🎥 [Watch the Competition Video](https://github.com/Josiah-Mechy/Ride-Engineering-Competition-2026/blob/main/Video.mp4)

---

# Competition Requirements

- Portable modular attraction
- Continuous operation during competition
- Safe passenger restraint system
- Structural integrity verification
- Complete engineering documentation
- Manufacturing within budget
- Professional presentation to industry judges

---

# Ride Features

- Chain lift hill
- Custom roller coaster vehicle
- 3D printed PLA track system
- Tapered helical descent
- 45° banked turn
- Magnetic braking section
- Arduino-based controls
- Break beam sensors
- PLC/HMI integration
- Modular support structure
- Portable frame for transportation

---

# My Contributions

## Mechanical Design

- Led the overall mechanical design of the attraction
- Designed the complete ride layout in SOLIDWORKS
- Designed the lap bar restraint system
- Designed the track, including the chain lift hill, helix, magnetic braking section, supports, and scenery
- Managed CAD integration across all subsystems
- Led design reviews and manufacturing readiness
- Performed the majority of engineering calculations in MATLAB and assisted with additional calculations in Microsoft Excel

## Engineering Analysis

Performed extensive engineering calculations and simulations including:

### Ride Dynamics

- Velocity profile
- Normal force analysis
- Lateral force analysis
- Resultant force analysis
- Total G-force analysis
- Sustained G-force duration
- Track loading calculations

### Structural Analysis

- ANSYS finite element analysis
- Support column stress analysis
- Flexure analysis
- Buckling analysis
- Fatigue analysis
- Lifetime analysis
- Factor of Safety calculations
- Tipping analysis
- Tubing structural verification

### Mechanical Systems

- Chain lift hill force analysis
- Chain lift Factor of Safety
- Sprocket loading analysis
- Motor torque calculations
- Motor power calculations
- Magnetic braking force analysis
- Lap bar material selection
- Lap restraint fatigue analysis

### Safety Engineering

- Rider restraint verification
- Station fencing analysis
- Safety factor validation
- Structural redundancy review

## Project Management

- Led an 11-member engineering team
- Coordinated project schedules and design milestones
- Managed subsystem integration
- Oversaw manufacturing progress
- Organized design reviews
- Prepared engineering documentation for competition

---

# Software & Tools

- SOLIDWORKS
- ANSYS Mechanical
- MATLAB
- Microsoft Excel
- Arduino IDE
- PLC Programming
- Bambu Studio

---

# Repository Contents

This repository contains the complete mechanical engineering package developed throughout the 2025–2026 Ride Engineering Competition season.

## CAD & Design

- **[Car Assembly v5.0 with Lap Bars.zip](https://github.com/Josiah-Mechy/Ride-Engineering-Competition-2026/blob/main/Car%20Assembly%20v5.0%20with%20Lap%20Bars.zip)**  
  Complete SOLIDWORKS assembly of the roller coaster vehicle, including the chassis, wheel assemblies, axle system, and lap bar restraint mechanism.

- **[Complete SolidWorks Track Assembly.png](https://github.com/Josiah-Mechy/Ride-Engineering-Competition-2026/blob/main/Complete%20SolidWorks%20Track%20Assembly.png)**  
  High-resolution rendering of the complete roller coaster track assembly and structural support system.

## Engineering Analysis

- **[Track_Data_V27.mlx](https://github.com/Josiah-Mechy/Ride-Engineering-Competition-2026/blob/main/Track_Data_V27.mlx)**  
  MATLAB Live Script containing the complete ride dynamics simulation, including velocity profiles, G-force calculations, normal and lateral force analysis, resultant forces, track loading, and ride performance validation.

- **[REC Graphs.xlsx](https://github.com/Josiah-Mechy/Ride-Engineering-Competition-2026/blob/main/REC%20Graphs.xlsx)**  
  Engineering calculations, supporting graphs, and validation data used throughout the design process.

- **[ANSYS Test Results - Small and Large Tubes.xlsx](https://github.com/Josiah-Mechy/Ride-Engineering-Competition-2026/blob/main/ANSYS%20Test%20Results%20-%20Small%20and%20Large%20Tubes.xlsx)**  
  Structural analysis results comparing multiple support tube configurations, including stress, deformation, buckling, and factor of safety calculations.

## Documentation

- **[TEA@LU Ride Engineering Competition Final Design Review.pptx](https://github.com/Josiah-Mechy/Ride-Engineering-Competition-2026/blob/main/TEA%40LU%20Ride%20Engineering%20Competition%20Final%20Design%20Review.pptx)**  
  Final Design Review presentation summarizing the engineering design process, structural validation, manufacturing, testing, competition readiness, and final results.

- **[REC 2026 ASTM Adaptation Document.pdf](https://github.com/Josiah-Mechy/Ride-Engineering-Competition-2026/blob/main/REC%202026%20ASTM%20Adaptation%20Document.pdf)**  
  Documentation detailing how the ride's design was adapted to meet applicable ASTM standards for the competition.

- **[REC 2026 Prompt.pdf](https://github.com/Josiah-Mechy/Ride-Engineering-Competition-2026/blob/main/REC%202026%20Prompt.pdf)**  
  The official competition prompt outlining the design challenge and requirements for the 2026 Ride Engineering Competition.

- **[TPE@LU ASTM F2291 Compliance.pdf](https://github.com/Josiah-Mechy/Ride-Engineering-Competition-2026/blob/main/TPE%40LU%20ASTM%20F2291%20Compliance.pdf)**  
  Compliance documentation verifying the ride's design against ASTM F2291 standards for amusement rides and devices.

- **[Ride Engineering Competition 2026 Rule Book.pdf](https://github.com/Josiah-Mechy/Ride-Engineering-Competition-2026/blob/main/Ride%20Engineering%20Competition%202026%20Rule%20Book.pdf)**  
  Official rule book governing competition requirements, judging criteria, and design constraints.

---

# Lessons Learned

This project significantly strengthened my experience in:

- Mechanical system design
- Engineering project leadership
- Finite Element Analysis (FEA)
- MATLAB engineering simulation
- Structural mechanics
- Dynamic systems analysis
- Ride safety engineering
- Design for manufacturability
- Engineering documentation
- Cross-disciplinary teamwork

---

# Electrical & Controls

This repository focuses on the **mechanical design, structural analysis, and engineering validation** of the project.

To view the electrical systems, Arduino programming, PLC implementation, sensor integration, and controls architecture developed for the ride, visit the team's electrical and controls repository:

### **https://github.com/TrialsVol2/2025-2026-Rollercoaster-SRCS**

---

# Credits

**Liberty University Theme Park Engineering Club**

**Ride Engineering Competition 2026**

**Design Team Lead & Project Manager:** Josiah Farinacci
